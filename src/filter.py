...
sensor_model = {
    0: {'mean': 0.0, 'std': 0.01}, 
    5: {'mean': 0.001562, 'std': 0.010016}, 
    10: {'mean': 0.001555, 'std': 0.009807}, 
    15: {'mean': 0.001779, 'std': 0.009260},
    20: {'mean': 0.001651, 'std': 0.009873},
    25: {'mean': 0.000387, 'std': 0.009871},
    30: {'mean': 0.001438, 'std': 0.010012},
    37: {'mean': 0.000625, 'std': 0.010058}
}
...
class ParticleFilter:
    def __init__(self, x, y, theta, map_file):
        ...
        self.translation_model = {  # duration to mean and std of speed (mm/s)
            ...
            2.5: {'mean': 0.020389545586205052, 'std': 0.0008114051601995619},
            5.0: {'mean': 0.02030169378258598, 'std': 0.0003927816070866799},
            7.5: {'mean': 0.02016625937015147, 'std': 0.0002745179877057256}
        }
    ...
    def rotate_particles(self, angle):
        for particle in self.particles:
            ...
            time = abs(angle / cfg["ROTATION_SPEED"])
            particle_angle = particle_w * time
            particle.rotate(particle_angle)
            distance = np.random.normal(self.rotation_translation_model[angle]['mean'], self.rotation_translation_model[angle]['std']) * time
    ...
    def convergence_rate(self):
        ...
        distances = np.array([np.sqrt((p.x-x_avg)**2 + (p.y-y_avg)**2) for p in self.particles])
        print(f"### In 0.05 of the average particles: {np.sum(distances <= 0.05) / cfg['PARTICLES'] * 100 :.2f}%")
    ...
    def run_filter(self):
        ...
        while not rospy.is_shutdown():
            ...
            print(f'&&& Robot move: {robot_move}, sensor_distance: {self.robot.sensor_distance}')
            print(f'&&& Explore state: {explore_state}')
...
def main():
    ParticleFilter(x=-0.175, y=0.175, theta=0, map_file=cfg["MAP_FILE"])
